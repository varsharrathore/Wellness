...
export default function LoginPage() {
  ...
  const handleRegister = async (data: RegisterForm) => {
    ...
    try {
      ...
      // Only send referralCode if it's not empty
      if (!registerData.referralCode?.trim()) delete registerData.referralCode;
      const response = await authAPI.register(registerData);
      login(response.data.token, response.data.user);
      // Show wallet bonus message if referral was used
              ...
              <form onSubmit={registerForm.handleSubmit(handleRegister)} className="space-y-6">
                ...
                <div>
                  ...
                  {registerForm.formState.errors.identifier && (
                    <p className="text-red-500 text-sm mt-1">
                      {registerForm.formState.errors.identifier.message}
                    </p>
                  )}
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Referral Code <span className="text-gray-400 font-normal">(optional — get ₹30 bonus!)</span>
                  </label>
                  <input
